...
- Follow [`.ai/workflow.md`](.ai/workflow.md) for non-trivial features (spec → slices → MCP → review).
- Role contracts: [`.ai/agents/`](.ai/agents/).
- Architecture hub: [`docs/architecture.md`](docs/architecture.md).
- Coding standards hub: [`docs/coding-standards.md`](docs/coding-standards.md).
- Multi-root workspace: open `../foro.code-workspace` (foro-web + foro-api).

## Overview

Foro is a React + TypeScript SPA (Vite dev server on port 5178) for invoice/quotation management.

**Backend:** sibling repo **`foro-api`** (Node + TypeScript + MySQL). Domain CRUD and auth go through `VITE_API_URL` via `foroApiClient`.
...
The `.env` file is gitignored. Without a reachable `VITE_API_URL`, the frontend will load but API calls will fail.
...
- **API client**: `ForoApiClient` has retry with exponential backoff (3 retries, jitter), 30s request timeout via AbortController, and GET request deduplication.
...
- More pitfalls: [`.ai/common-errors.md`](.ai/common-errors.md).
